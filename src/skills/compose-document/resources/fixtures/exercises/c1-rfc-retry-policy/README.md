# C1 RFC Retry Policy Exercise

This is a local, human-executed record of invoking `compose-document`, `shape-document`, `trace-artifact`, and `review-document`. It is not a parser, schema validator, publication event, or stakeholder approval.
...
1. `invocation-and-reexercise.md` records the actual local invocation and re-exercise sequence.
2. `baseline.md` freezes the retained current-workflow artifact and its historical SNAP-001 inputs before the SNAP-003 composition.
3. `selection-manifest.md` records composition selection, omissions, and the RFC basis.
4. `decision-brief.md` is the shaped Markdown candidate and `artifact.trace.md` binds its semantic units to SNAP-003.
