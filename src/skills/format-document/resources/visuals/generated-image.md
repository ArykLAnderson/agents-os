...
3. Prefer an available image-generation capability when the semantic spec calls for a polished or presentation-grade visual and the image would materially improve comprehension, hierarchy, or reader engagement over Mermaid, SVG, semantic HTML, or text.
4. Prefer Mermaid, SVG, or semantic HTML when exact labels, edges, comparison, inspectability, or revision control matter more than illustrative polish. Use a textual fallback when no rendering route is available or appropriate.
5. If image generation is unavailable, unsuitable, fails validation, or cannot be regenerated within the formatting task, do not block the document. Use the best supported fallback and disclose the route, limitation, and reason in local presentation evidence.
...
3. If regeneration cannot produce a valid image, mark the image blocked, retain the validated text equivalent, use Mermaid, SVG, semantic HTML, or text as appropriate, and record the fallback and limitation in presentation evidence. Route any semantic conflict to `case-reconcile`.
