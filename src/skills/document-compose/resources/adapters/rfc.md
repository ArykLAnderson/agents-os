...
**Structure mode:** recommended. Use the listed basis when it serves the reader; preserve semantic obligations when an established organizational template requires a different order.

## Reader Contract

- Name the intended technical reader and any Case-backed action requested: approve a direction, reject it, request a bounded follow-up, or review a proposal. Keep any externally supplied review instruction distinct from Case-backed meaning.
...
- When the selected direction is already accepted, do not ask the reader to approve it again. Either produce a decision record with no reader action, or record any externally supplied review instruction as context rather than Case authority.
- Use referenced Case state as the source of accepted meaning. Preserve the Case ID for every selected entry.

## Selection Manifest

Create `selection-manifest.md` beside the composition. Include:

- artifact ID, genre, reader action, Case IDs, and retained Case-state IDs when available;
...
Stop and report a blocking gap when the Case cannot support the stated decision, the action is ambiguous, selected entries conflict without a resolved Case decision, or a material assertion would require new accepted meaning. If the intended action would authorize implementation, ownership, schedule, rollout, or another fact absent from the Case, narrow the action or route a bounded question to `document-reconcile`; do not silently repair the gap in the RFC.
