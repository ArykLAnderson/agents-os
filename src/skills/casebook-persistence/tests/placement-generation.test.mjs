...
test("disposable SQLite atomically projects canonical P/R and owner-neutral query material", { timeout: 60_000 }, async (t) => {
  ...
  const init = { protocol: { id: "casebook-persistence-json", version: 2 }, operation: "initialize_store", request_version: 1, operation_id: "operation:placement-pr:bootstrap", store_id: ids.store, workspace_id: ids.workspace, authority_claim: { human_authorized: true, local_uid: process.getuid(), human_identity: "test", provenance: "disposable" }, configuration, initial: {
  ...
  const context = async (operation, extra) => { const value = { operation, operation_id: `operation:placement-pr:${operation}`, store_id: ids.store, workspace_id: ids.workspace, admission_slot_id: ids.slot, admission, configuration, ...extra }; value.request_digest = canonicalContextRequestDigest(ids.store, value); return invokeContextOperation(value); };
  ...
  const service = createPlacementGenerationFoundation(createSuccessorSqlitePlacementAdapter({ configuration, store_id: ids.store, workspace_id: ids.workspace, admission_slot_id: ids.slot, admission, mechanical_options: { admissionRegistry: registry } }));
  ...
  const found = await organizationalSearch({ store_id: ids.store, workspace_id: ids.workspace, admission_slot_id: ids.slot, admission, configuration, query: "needle", scope: "exact_namespace", namespace_id: ids.namespace, tags: [{ key: "phase", value: "current" }] });
  ...
  const alias = await resolveOrganizationalIdentity({ store_id: ids.store, workspace_id: ids.workspace, admission_slot_id: ids.slot, admission, configuration, selector: { namespace_id: ids.namespace, alias: "ALPHA" } });
  assert.equal(alias.ok, true); assert.equal(alias.result.status, "found"); assert.equal(alias.result.identity.id, ids.case);
  const resolve = (operation, extra) => invokeSuccessorMechanicalOperation({ operation, configuration, store_id: ids.store, workspace_id: ids.workspace, admission_slot_id: ids.slot, admission, ...extra });
  ...
  const second = createPlacementGenerationFoundation(createSuccessorSqlitePlacementAdapter({ configuration, store_id: ids.store, workspace_id: ids.workspace, admission_slot_id: ids.slot, admission, mechanical_options: { admissionRegistry: registry } }));
  ...
  const baseRequest = { protocol: { id: "casebook-persistence-json", version: 2 }, store_id: ids.store, workspace_id: ids.workspace, admission_slot_id: ids.slot, admission, configuration };
