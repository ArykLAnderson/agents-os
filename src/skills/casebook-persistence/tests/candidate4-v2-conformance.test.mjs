...
import { PROTOCOL_ID, PROTOCOL_VERSION, RESULT_ID, RESULT_VERSION, SCHEMA_ID, SCHEMA_VERSION, SELECTION_ID, SELECTION_VERSION, success } from "../shared/protocol.mjs";
...
test("Candidate-4 v2 manifests advertise only the accepted shared contracts and retained endpoint boundary", async () => {
  ...
  for (const manifest of [source, runtime]) {
    ...
    assert.deepEqual(manifest.selection, { id: "casebook-cli-selection", version: 2, authority: "explicit SQLite/Profile selection only" });
