...
async function initialize(binary, root, store, suffix) {
  ...
  return { protocol: request.protocol, configuration: request.configuration, store_id: values.store, workspace_id: values.workspace, admission_slot_id: values.slot, admission: { kind: "sqlite_profile", binding: { selection_id: values.selection, selection_revision_id: values.srev, profile_id: values.profile, profile_revision_id: values.prev, activation_fence: 1 } } };
}
...
test("graph query cursor binds every graph scope and fails on H/P/R change without exposing cursor fields", () => {
  ...
  const binding = queryBinding({ graph: "traverse", scope: "exact_namespace", namespace: id("namespace", 1), start: { kind: "knowledge", id: id("knowledge", 1) } });
