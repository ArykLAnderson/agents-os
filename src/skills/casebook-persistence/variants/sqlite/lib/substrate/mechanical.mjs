...
async function readOwnerRevision(request) {
  ...
  const owner = requireObject(request.owner, "owner");
  const kind = requireString(owner.kind, "owner.kind", 64);
  const targetId = requireUuidId(owner.id, "owner.id", kind);
  const revisionId = request.revision_id == null ? null : requireUuidId(request.revision_id, "revision_id", "owner-revision");
  if (revisionId == null && (!Number.isInteger(request.revision_number) || request.revision_number < 1)) throw new RequestError("identity_invalid", "A positive revision_number or revision_id is required.");
  const prepared = await prepare(request);
  if (prepared.failure) return prepared.failure;
  const { binary, storePath, state } = prepared;
  if (request.store_id !== state.metadata.store_id) return failure("not_visible", "The requested revision is unknown or not visible.", { failureClass: "not_visible", evidence: {} });
  const view = await validateActiveView(binary, storePath, state, context);
  if (view.failure) return view.failure;
  const selector = revisionId == null ? `r.revision_number = ${request.revision_number}` : `r.revision_id = ${sqlText(revisionId)}`;
  const rows = await queryJson(binary, storePath, `
    ...
    JOIN view_policy_namespace_grants grant ON grant.namespace_id=o.home_namespace_id AND grant.view_policy_revision_id=${sqlText(context.view_policy_revision_id)}
    JOIN view_policy_revisions vpr ON vpr.view_policy_revision_id=grant.view_policy_revision_id AND vpr.view_id=${sqlText(context.view_id)} AND vpr.lifecycle='active'
    JOIN json_each(vpr.object_kinds_json) object_kind ON object_kind.value=o.owner_kind
    WHERE o.owner_id=${sqlText(targetId)} AND o.owner_kind=${sqlText(kind)} AND ${selector} LIMIT 1;`);
  if (!rows.length) return failure("not_visible", "The requested revision is unknown or not visible.", { failureClass: "not_visible", evidence: {} });
  const row=rows[0];
  ...
  if(selected.length>MAX_SELECTIONS)return failure("representation_invalid","The owner revision exceeds the bounded read shape.",{failureClass:"representation_invalid",retryDisposition:RETRY_DISPOSITIONS.AFTER_OPERATOR_REPAIR,evidence:{}});
  return success("read_owner_revision",{status:"found",owner:{id:row.owner_id,kind:row.owner_kind,home_namespace_id:row.home_namespace_id},revision:{id:row.revision_id,number:row.revision_number,normalized:JSON.parse(row.normalized_json),representation:{id:row.representation_id,version:row.representation_version},selected_versions:selected.map(item=>({family_id:item.family_id,version_id:item.version_id,content:JSON.parse(item.content_json),content_digest:item.content_digest})),committed_at:row.committed_at},applied_view:{view_id:context.view_id,view_policy_revision_id:context.view_policy_revision_id}});
}
