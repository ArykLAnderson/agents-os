...
function resourceProjectionStatements(envelope, now) {
  const delta = envelope.resource_delta;
  if (!delta) return [];
  const owner = envelope.owner;
  const revision = envelope.revision;
  const resourceIdentitySet = delta.resources.map((resource) => sqlText(resource.resource_id)).join(",") || "NULL";
  const statements = [
    `CREATE TEMP TABLE preserved_inbound_relationships AS SELECT * FROM relationship_current WHERE owner_id<>${sqlText(owner.id)} AND source_resource_id IN (${resourceIdentitySet});`,
    `DELETE FROM relationship_current WHERE owner_id<>${sqlText(owner.id)} AND source_resource_id IN (${resourceIdentitySet});`,
    `DELETE FROM relationship_current WHERE owner_id=${sqlText(owner.id)};`,
    `DELETE FROM resource_search_fts WHERE resource_id IN (SELECT resource_id FROM resource_search_current WHERE owner_id=${sqlText(owner.id)});`,
    `DELETE FROM resource_search_current WHERE owner_id=${sqlText(owner.id)};`,
    `DELETE FROM resource_current WHERE owner_id=${sqlText(owner.id)};`,
  ];
  if (owner.kind === "case") {
    statements.push(`DELETE FROM case_alias_current WHERE case_id=${sqlText(owner.id)};`);
    if (envelope.current_projection.state === "active") for (const alias of envelope.current_projection.aliases ?? []) {
      statements.push(`INSERT INTO case_alias_current(namespace_id,normalized_alias,case_id,owner_revision_id,display_alias) VALUES(
        ${sqlText(owner.home_namespace_id)},${sqlText(alias.normalized_value)},${sqlText(owner.id)},${sqlText(revision.id)},${sqlText(alias.value)}
      );`);
    }
  }
  for (const resource of delta.resources) {
    statements.push(
      `INSERT INTO resource_current VALUES(
        ${sqlText(resource.resource_id)},${sqlText(resource.resource_kind)},${sqlText(owner.id)},${sqlText(revision.id)},${revision.number},
        ${sqlText(resource.family_id)},${sqlText(resource.version_id)},${sqlText(resource.lifecycle)},${sqlText(JSON.stringify(resource.projection))},${sqlText(now)}
      );`,
    );
    if (resource.lifecycle === "active" && resource.search) {
      statements.push(
        `INSERT INTO resource_search_current VALUES(
          ${sqlText(resource.resource_id)},${sqlText(owner.id)},${sqlText(revision.id)},${sqlText(resource.resource_kind)},
          ${sqlText(resource.search.text)},${sqlText(JSON.stringify(resource.search.metadata))}
        );`,
        `INSERT INTO resource_search_fts(resource_id,search_text) VALUES(${sqlText(resource.resource_id)},${sqlText(resource.search.text)});`,
      );
    }
  }
  for (const relationship of delta.relationships) {
    statements.push(`INSERT INTO relationship_current VALUES(
      ${sqlText(relationship.relationship_id)},${sqlText(owner.id)},${sqlText(revision.id)},${sqlText(relationship.source_resource_id)},
      ${sqlText(relationship.target.kind)},${sqlText(relationship.target.id)},${sqlText(relationship.predicate)},${sqlText(JSON.stringify(relationship.metadata))}
    );`);
  }
  statements.push("INSERT INTO relationship_current SELECT * FROM preserved_inbound_relationships;", "DROP TABLE preserved_inbound_relationships;");
  if (process.env.CASEBOOK_PERSISTENCE_TEST_FAULT === "after_resource_projection_before_event") {
    statements.push("INSERT INTO synthetic_resource_projection_fault VALUES(1);");
  }
  return statements;
}
...
function buildCommitSql(state, context, envelope, coreResult, now) {
  ...
  statements.push(...resourceProjectionStatements(envelope, now));
  statements.push(
    "UPDATE store_fence SET operation_fence = operation_fence + 1 WHERE singleton = 1;",
...
async function commitOwnerRevision(request) {
  ...
  const coreResult = {
    ...
    allocations: {
      ...
      resource_ids: envelope.resource_delta?.resources.map((item) => item.resource_id) ?? [],
    },
    ...(envelope.resource_delta ? { resource_projection: { operation_fence: state.operation_fence + 1 } } : {}),
...
async function readOwnerRevision(request) {
  ...
  return success("read_owner_revision",{status:"found",owner:{id:row.owner_id,kind:row.owner_kind,home_namespace_id:row.home_namespace_id},revision:{id:row.revision_id,number:row.revision_number,normalized:JSON.parse(row.normalized_json),representation:{id:row.representation_id,version:row.representation_version},selected_versions:selected.map(x=>({family_id:x.family_id,version_id:x.version_id,content:JSON.parse(x.content_json),content_digest:x.content_digest})),committed_at:row.committed_at},applied_view:{view_id:context.view_id,view_policy_revision_id:context.view_policy_revision_id},operation_fence:row.operation_fence});
}
...
async function readActiveViewScope(request) {
  ...
  return success("read_active_view_scope", {
    ...
    operation_fence: rows[0]?.operation_fence ?? state.operation_fence,
  });
}
...
async function purgeCasePayload(request) {
  ...
  const command = `.bail on
    ...
    DELETE FROM relationship_current WHERE owner_id=${sqlText(targetCaseId)};
    DELETE FROM resource_search_fts WHERE resource_id IN (SELECT resource_id FROM resource_search_current WHERE owner_id=${sqlText(targetCaseId)});
    DELETE FROM resource_search_current WHERE owner_id=${sqlText(targetCaseId)};
    DELETE FROM case_alias_current WHERE case_id=${sqlText(targetCaseId)};
    DELETE FROM resource_current WHERE owner_id=${sqlText(targetCaseId)};
