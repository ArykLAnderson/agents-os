...
function admission(request) { if (!ID.test(request.store_id ?? "") || !ID.test(request.workspace_id ?? "") || !ID.test(request.admission_slot_id ?? "")) throw new GraphError("graph_request_invalid", "store, workspace, and admission identities are required."); return prepareAdmission({ registry: FINAL_ADMISSION_REGISTRY, operation: "query.graph", workspaceId: request.workspace_id, admissionSlotId: request.admission_slot_id, admission: request.admission }); }
...
export async function graphQuery(request) {
  ...
  try {
    if (request.store_id !== p.state.metadata.store_id || request.workspace_id !== p.state.metadata.workspace_id) return failure("store_target_mismatch", "The immutable store/workspace identity differs.");
