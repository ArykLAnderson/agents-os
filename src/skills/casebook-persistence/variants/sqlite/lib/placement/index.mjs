...
export function createSuccessorSqlitePlacementAdapter(binding) {
  if (!exact(binding, ["admission", "admission_slot_id", "configuration", "mechanical_options", "store_id"]) || (binding.mechanical_options != null && !exact(binding.mechanical_options, ["admissionRegistry"]))) throw new PlacementError("placement_adapter_invalid", "SQLite placement binding must be exact.");
  const base = Object.freeze({ configuration: binding.configuration, store_id: identity(binding.store_id, "store_id", "store"), admission_slot_id: identity(binding.admission_slot_id, "admission_slot_id", "admission-slot"), admission: binding.admission });
