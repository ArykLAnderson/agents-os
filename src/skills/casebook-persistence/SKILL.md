...
All exact view-policy checks, bounded scans/cursors, immutable versions, receipts, and recovery classifications are mechanical. Persistence never decides knowledge classification, Case disposition, reconciliation meaning, Frame completion, publication, or external effects. Provider-local SQLite organizational lexical search is supported through `query.search`, `query.resolve`, and `query.hydrate`, including its selected-authority global scope. Graph, semantic, hybrid, and final-CLI query remain unsupported, as do general-purpose and non-disposable migration/snapshot/restore. Trusted-local cursor and handoff integrity detect accidental tampering; they are not hostile-client authentication.
