...
const findings = await parallel("review", {
  architecture: () => agent(prompt(
    ...
    { draft },
  ), { label: "Architecture review", model: "workhorse", tools: ["read", "bash"] }),
  validation: () => agent(prompt(
    "Check the candidate against the exact accepted criteria. Return PASS or bounded defects; do not repair it.\n\nCandidate:\n{draft}",
    { draft },
  ), { label: "Independent validation", model: "workhorse", tools: ["read", "bash"] }),
});
return await agent(prompt(
  ...
  { draft, findings },
), { label: "Candidate owner reconciliation", model: "involved", tools: ["read", "bash"] });
```
...
return await agent(prompt(
  ...
  { evidence },
), { label: "Evidence synthesis", model: "workhorse", tools: ["read"] });
```
...
return await withWorktree("slice", async ({ path, branch }) => {
  ...
  const review = await agent(prompt(
    ...
    { implementation },
  ), { label: "Advisory review", model: "workhorse", tools: ["read", "bash"] });
