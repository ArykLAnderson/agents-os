...
Blueprint and Software Implementation call Feature Atlas domain operations. They do not ad hoc run provider commands, enumerate records, parse an unselected provider layout, infer a destination, select Git branches, or treat raw command output, mutable files, or paths as currentness authority. When a configured adapter is supplied as executable instructions rather than a separate runtime wrapper, the domain consumer may execute its documented provider commands directly as adapter-owned mechanics, or its documented filesystem reads, only as part of the complete named operation, preserving fail-closed checks and returning the semantic classification plus receipt. For the selected local filesystem adapter, follow [the configured local filesystem adapter](configured-local-filesystem.md); for private GitHub Issues, follow [the configured private GitHub adapter](configured-private-github.md). Publisher may use domain mutation operations only under exact authority.
