...
There is no generic `route` admission mode. A legacy Route package, candidate, ticket graph, summary, or hand-written imitation cannot acquire Atlas authority here. Admit accepted feature delivery only from the current accepted Atlas Map. Read `.casebook/atlas-method.md` and follow its Markdown or GitHub Issues guide with ordinary tools. If it is absent, use Feature Atlas setup to ask the user for method and destination, record the answer there, then read the configured Atlas before admission. `CASEBOOK_DATABASE_URL` is unrelated.

For `atlas`, persist the compact Delivery Binding defined in [Portable Contracts](references/contracts.md): publication method/destination, Map and Decision, Blueprint revision, selected Feature/Work Item ownership and outcomes, prerequisites and convergence, obligations and proof responsibilities, limitations/source links, fresh reread result, and explicit absent/present authorities. Refuse a summary, historical Decision, `HandoffRefusal`, incomplete/conflicted projection, omitted authority, or unverifiable binding. A `HandoffWithLimitations` is admissible only to the exact boundary its typed limitations and separately explicit implementation authority allow; affected work remains blocked.
