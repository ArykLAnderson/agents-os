...
Copy this from the published Atlas handoff:

```markdown
Handoff: HandoffReady | HandoffWithLimitations
...
Authority boundary: <present/absent implementation, effects, PR, merge, deploy and landing authority>
Fresh reread: <time, records read, complete and consistent | exact problem>
```

`HandoffReady` is not dispatch or effect authority. `HandoffWithLimitations` is admissible only for its explicitly unaffected work. Refuse a historical Decision, incomplete or conflicting publication, unresolved ownership/dependency, or omitted authority.

## Atlas Currentness Check

At admission, coordinator resume, before each dependency frontier, before an effectful gate, and before result, reread the bound Map and Decision through the guide selected by `.casebook/atlas-method.md`. Confirm that the Decision remains current and the selected work, ownership, prerequisites, limitations, and source links still agree. Never follow a successor automatically.

```markdown
Checkpoint: admission | resume | dependency_frontier | effectful_gate | result
Bound / observed Decision: <IDs and locators>
Records read: <Map, Feature and Work Item paths or Issue URLs>
Result: clear | exact_admitted_limitation | stop
Affected execution: <allowed boundary or stopped work>
Observation time: <time>
```

Stop for an unaccounted successor, conflict, unreadable publication, changed accepted meaning, triggered invalidator, or omitted authority. Keep only the latest check and material stop evidence in the execution map.

## Task Contract

```markdown
Task: <stable execution identity; exact WI-* binding in atlas mode>
Outcome/why: <bounded result and consumer value>
Atlas owner/Decision: <Feature and current Map Decision, or not applicable>
