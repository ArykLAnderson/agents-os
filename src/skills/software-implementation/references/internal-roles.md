...
> Use only the derived draft-PR operation binding from the supplied Execution Authorization Envelope; no second human confirmation is required for operations already inside that binding. Inspect the validated direct-task delivery branch or applicable global/Feature integration branch and current gate evidence; prepare a reader-oriented summary with tests, reviews, E2E, limitations, and non-blocking findings. Push only the exact bound head with a non-force update, then query the bound provider/account/repository for an open PR matching the same head and declared base. Return it if found; otherwise create a draft PR when authorized. Update only the matching owned draft PR within the declared stack graph. Return the prepared summary and verified branch if push or creation is unavailable or fails. Never mark ready, assign reviewers, mutate labels/projects, retarget outside the graph, or force-push. Never merge, mutate a protected branch, deploy, or perform external/live effects.
