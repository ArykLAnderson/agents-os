...
Use `casebook read frame --frame-id <stable-id>` to resume a Frame. The optional `--owner-revision-id` reads a named revision. There is no ordinary public Frame list, resolve, Discovery, disposition, or history command. Use `casebook search --namespace <semantic-id> --query <text>` only to find candidates, then read a returned stable Frame ID; search does not establish identity.

For a new Frame, use direct flags for simple useful creation, or pipe compact authoring JSON to valueless `--draft` for richer creation. Submit once with `casebook create frame --namespace <semantic-id> --commit-basis <basis> ...`; do not use a file as the normal authoring path. `--input-file` remains a compatibility/fallback transport when needed. For a change, first read the exact complete aggregate, preserve every unchanged family and stable ID, then submit it once through `casebook commit frame --namespace <semantic-id> --frame-id <id> --expected-revision <current-number> --commit-basis <basis> --input -`. Commit accepts the full aggregate transport only; direct authoring flags and `--draft` are creation-only. Namespace selects request placement only and never relocates existing content implicitly. The CLI accepts one complete aggregate through `--input`, standard input via `--input -`, or compatibility/fallback `--input-file`; use exactly one mode for a full-aggregate mutation and none for a read or search.

To delete a Frame, first read it exactly, then use `casebook delete frame --namespace <semantic-id> --frame-id <id> --expected-revision <current-number> --reason <text>`. This is revision-checked logical tombstoning: ordinary current reads/search hide the Frame while durable history and receipts remain; it is not physical purge. Do not treat tombstoning an individual Discovery or knowledge item as deleting its whole owner.
