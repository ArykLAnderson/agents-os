...
1. If `CASEBOOK_DATABASE_URL` is set in the environment, use SQLite authority. Resolve the value to an absolute local path. This is the database path for the SQLite connector. Read the database's `store_authority_binding` to obtain the store ID, then read the active view and policy-revision IDs from the connector's identity discovery surface. No additional external configuration is required.
2. Otherwise, use Markdown authority. Resolve the workspace root from an explicit user-provided path when present, or the current project's private `.casebook/` workspace. Convert to an absolute path. A Markdown workspace must have an exact `.casebook-authority.json` marker.
3. Select only the matching connector: `variants/sqlite/bin/casebook-persistence.mjs` for SQLite or `variants/markdown/bin/casebook-persistence.mjs` for Markdown. Invoke it with an explicit Node interpreter and one version-1 JSON request on stdin. Use the connector beside the loaded generated skill, never a source-tree or live-sync fallback.
4. If the resolved database does not exist, the environment variable points to an invalid path, or the Markdown workspace lacks an authority marker, stop without reading or writing Frame or Case content. Do not probe both connectors, hot-switch, initialize, migrate, import, mirror, or repair configuration during an ordinary Frame operation.

Every request carries the selected configuration and its provenance, protocol `casebook-persistence-json` version `1`, request version `1`, selected store ID, exact view and policy-revision IDs, a narrow purpose, and `requested_audience_ceiling: "private"`. The two authority modes are mutually exclusive; there is no fallback or dual write. Never write `.casebook/frames/` directly, never parse a guessed Frame path as authority, and never treat Markdown as a writable projection of SQLite.
