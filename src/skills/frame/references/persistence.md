...
For a new Frame, submit one complete aggregate with `casebook create frame --namespace <semantic-id> --commit-basis <basis> --input-file <absolute-file>`. For a change, first read the complete aggregate, preserve unchanged families and stable IDs, then submit it once with `casebook commit frame --namespace <semantic-id> --frame-id <id> --expected-revision <current-number> --commit-basis <basis> --input-file <absolute-file>`. Namespace selects request placement only and never relocates existing content implicitly. The CLI accepts one complete aggregate through `--input`, standard input via `--input -`, or `--input-file`; use exactly one mode for a mutation and none for a read or search.
