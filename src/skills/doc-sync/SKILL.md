---
name: doc-sync
description: Keep project documentation in sync with code changes. Updates API references, schema docs, and architectural overviews after implementation. Invoked by an authorized implementation workflow or standalone after manual changes.
user-invocable: true
...
1. **Determine what changed**: If invoked by an authorized implementation workflow, inspect its assigned implementation/integration diff. If standalone, use `git diff` against the base branch (or `$ARGUMENTS` for specific files).
