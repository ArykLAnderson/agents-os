...
Create `SNAP-001` only after explicit author approval or correction resolves every material item required for the initial binding Case state. The retained accepted state is not a review queue record or a copy of the mutable working ledger.
...
- **Approval:** APR-001
- **Entries:** `snapshots/SNAP-001.entries.md`
- **Supersedes:** none
- **Artifacts:** none
```
...
The entry manifest is retained accepted-state content, not an ID list. Store each included entry in recorded order using its complete, normalized Markdown representation, including stable ID, label, statement, status, classification, provenance, source locators, approval reference when applicable, and semantic fields. Do not rewrite it as part of later working-ledger changes. A later correction, supersession, or source update creates a later entry and accepted-state record; it cannot silently rewrite `SNAP-001`.
