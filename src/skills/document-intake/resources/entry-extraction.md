...
Normalize registered source content into a working Case ledger after source registration. This operation creates candidate semantic entries and explicit gaps. It does not create `SNAP-001` or adopt binding meaning without the author.
...
IDs use a core-type prefix and stable sequential number. Keep the ID when a label or statement receives a non-semantic correction. Do not store source passages, transcript summaries, document sections, or reader-facing prose as entries.

## Controlled Types And Statuses

Use only these core types. Prefer a namespaced `Subtype` to a new top-level type. `Status` describes the type's lifecycle; `Classification` uses the universal Case contract and describes whether current document work may rely on the entry.
...
Classify an entry as `accepted` only when settled enough for the current purpose; `provisional` when it needs confirmation before consequential use; `contested` when material sources or participants disagree; and `superseded` when later accepted context replaces it while it remains explanatory history. Preserve provenance separately.
