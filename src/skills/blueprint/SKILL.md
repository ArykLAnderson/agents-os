...
**Blueprint lens:** inspect current terrain and select one coherent responsibility/contract/state design. Name canonical owner for material state, mutation, contracts/schemas, and recovery/operation where relevant. A **material consumer seam** is a public boundary or internal callable/projection handoff whose shape can change behavior, authority/trust, persistence or replay, failure/recovery, compatibility, or the selected route. Before treating one as closed, run the [consumer-seam interrogation](references/requirement-killer.md#interrogate-contract-burden) for each distinct immediate consumer, or establish that several consumers share the same projection. Record the result as linked atomic Case claims rather than a new ledger, then run an analytical immediate-consumer walkthrough through that exact shape. Exercise a real call or projection only when decision-changing evidence remains, using the smallest disposable probe.

Apply the design side of the [Requirement-Killer Pass](references/requirement-killer.md) after architecture selection and consumer-seam interrogation: challenge unnecessary state, owners, seams, abstractions, contracts, custom machinery, duplicate responsibilities, and leaked prototype scaffolding; prefer existing mechanisms. Reconcile the dispositions into the Case, then rerun the immediate-consumer, representative applicable-failure, and fresh-worker walkthroughs against the pruned design.
...
- **Fresh-worker executable candidate:** every consequential claim is selected, evidence-dependent claims are supported, owners/consumers/failures are closed, every material consumer seam has passed consumer-seam interrogation, the current [Requirement-Killer Pass](references/requirement-killer.md) is reconciled, and a fresh worker walkthrough finds no architecture invention.
...
Only after routeability, invoke `../document/SKILL.md` to materialize `templates/production-design-rfc.md` as a **lossless reader-facing semantic projection** from one fixed Design/RFC Case revision, including any Presentation handoff required by the requested artifact. When a rendered visual or interactive review is requested, invoke `../presentation/SKILL.md`; its optional [Art Direction lens](../presentation/references/art-direction.md) is subordinate to that work. Follow the narrative and commissioning guidance in `templates/production-design-rfc.md`. The Case remains the sole semantic design authority: a semantic discrepancy or requested change returns to the Case, which fixes a new revision before rematerialization. Review findings reconcile into Case claims, never directly into the RFC. Only an accepted fresh-worker-executable RFC projection may proceed to Atlas.
...
3. analytical immediate-consumer and representative applicable failure walkthroughs of the exact material seam shapes, plus consumer-seam-interrogated contracts, owners, failures, and route with no caller or worker invention;
...
- a private semantic RFC whose prose and meaning-bearing visual specifications make the architecture clear to the founder returning cold, remain traced to the fixed Case, and include a complete Presentation handoff when a rendered surface is requested; any basic desktop/phone readability check is Presentation evidence;
- one causal explanatory spine from situation and tension through decisive move, complete instance, revealing failure, realization route, and acceptance decision, independently retold by a fresh-context reader without hidden context or architecture invention;
