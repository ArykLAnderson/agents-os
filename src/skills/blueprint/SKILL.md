...
Only after routeability, invoke `../document/SKILL.md` to materialize `templates/production-design-rfc.md` as a **lossless reader-facing projection** from one fixed Design/RFC Case revision. The default acceptance package is a private, self-contained HTML RFC for the founder returning cold: editorial clarity, causal explanation, and comfortable desktop/phone reading dominate. Before choosing sections or visuals, fix one explanatory spine: situation → tension → decisive architectural move → consequent shape → one complete instance story → one revealing stress story → realization route → acceptance decision. Every primary section must inherit a question from the previous one and advance that spine; coverage that does not belong in the story moves to a compact appendix. Commission visuals only for distinct spatial, temporal, structural, or comparative reader questions—never to satisfy section quotas or prose cadence. Follow the narrative and commissioning guidance in `templates/production-design-rfc.md`; visuals project the Case and introduce no new design meaning. For the default private review artifact, perform one basic browser-open/render check at ordinary desktop and phone widths; do not require bespoke ARIA, accessibility audits, zoom matrices, print/PDF inspection, multi-browser testing, or publication hardening unless the user requests it or the stated audience/distribution boundary makes it material. In this bounded invocation, the Case remains the sole semantic design authority: Document may govern editorial organization, prose, trace, review, and representation, but may not add, select, reinterpret, or change a semantic design claim. Any semantic discrepancy or requested semantic change returns to the Case first; fix and re-reconcile its claims, fix a new revision, then rematerialize. Review findings reconcile into Case claims, never directly into the RFC. Only an accepted fresh-worker-executable RFC projection may proceed to Atlas.
...
- a private HTML RFC whose prose and diagrams make the architecture clear to the founder returning cold, remain traced to the fixed Case, and pass a basic desktop/phone readability check;
