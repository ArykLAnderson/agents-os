...
When a candidate is motivated by security or assurance, include a structurally constrained alternative that makes the unsafe operation unavailable rather than predicting and pre-validating every execution path. Do not select manifests, snapshots, preflight phases, complete-capability enumeration, or request-wide fail-closed behavior without concrete evidence that structural restriction and local non-applicability are insufficient.
