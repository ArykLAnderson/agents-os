...
Editorial collaboration remains inside the same session. Feedback that changes meaning, reader action, claim strength, scope, structure, evidence, risk treatment, visual semantics, appendix boundaries, or publication state is a material edit: resume the session, return a completed Document to `active`, revise the authoritative shaped draft first, and invalidate every affected downstream state before changing a published representation. Purely visual or interactive feedback belongs to Presentation; meaning-changing pressure returns here before realization changes. Copyedits that preserve those dimensions do not require a lifecycle transition.

`Artifact Boundary` records requested genre, required semantic representations, any requested Presentation surfaces, and whether publication was requested. The semantic basis is pre-shaping material. Once a shaped draft exists, it is the sole authoritative semantic draft; trace, review, semantic representations, realization handoffs, acceptance, and publication name its revision. A material draft change invalidates affected downstream records.

The session owns editorial intent, the semantic draft, artifact-local semantic findings, semantic representation specifications, realization handoffs, and factual publication state. Presentation owns realized surfaces, rendered findings, inspection, and render readiness; link those facts without making them Document meaning. Cases own reusable subject meaning. Frames own broader uncertainty. Do not persist operation requests, callbacks, generic result logs, or recommended next actions.
